# Task 0040: The rules that can be checked become scripts, and two hooks run them

Status: todo
Depends on: —
Parallel: yes — alongside 0041, which owns the prose half of the same decision.
Their file scopes are disjoint: this task writes `scripts/`, `.husky/` and
`.claude/`; 0041 writes `CLAUDE.md` files and touches no script.
...
- [`decisions/0045`](../decisions/0045-rules-are-executed.md) — why a checkable
...
- `scripts/lint-landing.ts` (new) + test — 0033 §1 and 0045 §5.
...
- **Everything `CLAUDE.md`.** Task 0041 owns the directory files and the removal
...
- **Running `lint:records` and `lint:landing` in CI.** Both read the staged diff
  ...
  deferral to revisit: the two are pre-commit checks by construction, and 0045 §2
  already accepts that `--no-verify` reaches past them.
- **Enforcing where the root `CLAUDE.md` lands.** 0045 §5 names `.husky/`,
...
- **Checking that a deferral names an issue** (0042 §2). "Nobody wants it yet" and
  ...
  that names neither. It stays prose, and 0041 puts it where it is read.
- **Branch protection on main.** 0045's `Out of scope`, unchanged.
...
4. **Red — `scripts/lint-landing.ts`.** Export
   ...
   - **`decisions/CLAUDE.md` and `tasks/CLAUDE.md` are the exception to both
     ...
     not records: 0045 §5 lands a directory `CLAUDE.md` through review, and 0032
...
- [ ] `decisions/CLAUDE.md` commits on a `task/*` branch and fails on `main` —
      the opposite of `decisions/0045-*.md`, which does the reverse
...
- **Manual, against a real repository**: the wiring, which no unit test reaches.
  ...
  2. `git commit --no-verify` past the decision guard, confirming 0045 §2's
  ...
  4. **`pre-commit` wall-clock time, before and after**, recorded in the outcome
     notes. 0045's Consequences commit to measuring and then cutting what hurts;
