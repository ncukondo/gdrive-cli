# Task 0049: A `PRUNE_REQUIRED` refusal carries the plan it refused

Status: todo
Depends on: —
Parallel: yes (worktree-safe) — beside 0047, 0048 and 0050; it owns
...
- `src/commands/forms/write.test.ts`, `src/commands/slides/write.test.ts`
- `docs/commands.md` — the two `PRUNE_REQUIRED` paragraphs (around lines 1567
  and 2023) and the exit-code table's neighbourhood
