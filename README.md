...
- **A name you can address afterwards** — every command that gives a file a name
  refuses one the path to it would not bring back: one the folder already holds,
  one ending in a space, one containing `/`, and — at the top of a drive, where
  the name is the whole path — one spelled like the root, like an ID, or with a
  `drive:` prefix. Drive allows every one of them and leaves you a file no path
  can reach; this says so at the moment it happens, and names a replacement it
  has checked ([details](docs/commands.md#a-name-has-to-be-addressable)).
